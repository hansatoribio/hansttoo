...
const sections = {
  en: [
    ...
    ['Service providers and measurement', 'Form information may be processed and stored by the website’s database and file-storage provider, Supabase. Campaign source, medium, campaign name, ad content, search term, landing path, and supported click identifiers may be kept with the consultation so Hans can understand which advertising produced qualified requests. The site does not retain full referrer URLs or arbitrary query parameters for this purpose. If measurement is configured, Google tags use Consent Mode v2 and Meta Pixel loads only after you allow measurement. Google may receive limited cookieless consent and event signals when optional storage is denied. The form does not send your name, contact details, tattoo description, or uploaded images to advertising platforms.'],
  ...
  es: [
    ...
    ['Proveedores y medición', 'La información del formulario puede ser procesada y almacenada por Supabase, proveedor de base de datos y archivos del sitio. El origen, medio, nombre de campaña, contenido del anuncio, término de búsqueda, página de entrada e identificadores de clic compatibles pueden guardarse con la consulta para que Hans entienda qué publicidad genera solicitudes calificadas. Para este fin no se conservan URLs completas de referencia ni parámetros arbitrarios. Si se configura la medición, las etiquetas de Google usan Consent Mode v2 y Meta Pixel solo se carga después de que permitas la medición. Google puede recibir señales limitadas y sin cookies sobre el consentimiento y los eventos cuando se rechaza el almacenamiento opcional. El nombre, los datos de contacto, la descripción y las imágenes no se envían a plataformas publicitarias.'],
